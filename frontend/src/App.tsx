...
import {
  LogIn, UploadCloud, BarChart3, Brain, LineChart, Shield,
  Gauge, Sparkles, X, CheckCircle2
} from "lucide-react";
...
import ReportExampleBox from "./components/ReportExampleBox"; // <- блок примера отчёта

export default function App() {
  return (
    <Routes>
      <Route path="/" element={<Landing />} />
      <Route path="/dashboard" element={<AccountDashboard onLogout={() => window.location.assign("/")} />} />
      <Route path="/metrics/:videoId" element={<MetricsPage />} />
    </Routes>
  );
}
...
function Landing() {
  ...
  return (
    <div className="min-h-screen text-foreground overflow-x-clip">
      <StyleBrand />
      <GlobalBackground /> {/* фиксированный фон на всех скроллах */}

      <SiteHeader
        nav={[
          { label: "Продукт", href: "#upload" },
        ...
        rightSlot={
          <>
            <a
              href="#upload"
              className="inline-flex items-center gap-2 rounded-2xl px-5 py-2 font-semibold text-white shadow-md"
              style={{ backgroundImage: "var(--btn-grad)" }}
            >
              Проверить рекламу
            </a>
            <button
              onClick={() => { setPendingAction("none"); setShowReg(true); }}
              className="inline-flex items-center gap-2 rounded-2xl px-4 py-1.5 border border-white/15 hover:border-white/30 text-sm transition-colors"
            >
      ...
      {/* Модалки */}
      <AuthModal open={showReg} onClose={() => setShowReg(false)} onSubmit={handleAuthSubmit} />
      <ChangePlanModal
        open={showChangePlan}
        ...
        preset={chosenPlan}
      />
      <AddCardModal open={showAddCard} onClose={() => { setShowAddCard(false); navigate("/dashboard"); }} />
    </div>
  );
}

/* ---------- Глобальные стили и фиксированный фон ---------- */
function StyleBrand() {
  return (
    <style>{`
      :root{
      ...
      html,body,#root{min-height:100%;background:transparent}
      ...
      style={{
        background:
          `radial-gradient(1400px 800px at 20% -10%, rgba(107,60,255,0.35), transparent 65%),
           ...
           linear-gradient(180deg,#0b0b12,#111119)`,
...
/* ---------- Above the fold (без локального градиента) ---------- */
function AboveTheFold({ onRequireAuth }:{ onRequireAuth:()=>void }){
  return (
    <section id="above" className="min-h-[calc(100vh-64px)] flex flex-col justify-center relative overflow-hidden">
      <Hero />
      {/* Блок примера отчёта сразу под текстом, до загрузки */}
      <div className="mx-auto max-w-6xl px-5 -mt-2 mb-6">
        <ReportExampleBox pdfUrl="/report_example.pdf" />{/* положите файл в frontend/public/report_example.pdf */}
      </div>
...
function Hero() {
  return (
    <section className="text-center pt-16 pb-10 px-5">
      ...
      <p className="mt-4 text-lg md:text-xl text-gray-200/85 max-w-3xl mx-auto">
        SynapSight анализирует видеоролик и прогнозирует <span className="font-semibold">нейроэффективность</span> на основе моделей внимания, вовлеченности и запоминаемости.
      </p>
    </section>
  );
}
...
function ScrollHint(){
  ...
  return (
    <div className={`text-center transition-opacity ${visible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <span className="text-[11px] opacity-70">Прокрутите вниз</span>
      <span className="block w-2.5 h-2.5 rounded-full bg-white/80 mx-auto mt-1 animate-bounce" />
    </div>
  );
}
    ...
    <section className="mx-auto max-w-6xl px-5 py-20 text-center">
      <div className="card rounded-3xl p-10">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <a href="#pricing" className="px-6 py-3 rounded-2xl text-white font-semibold" style={{ backgroundImage:'var(--btn-grad)' }}>Проверить рекламу сейчас</a>
        ...
        {mode === 'success' ? (
          ...
          <>
            ...
            <form onSubmit={submitLogin} className="space-y-3">
              ...
              <button disabled={loading} className="w-full rounded-xl px-4 py-2 font-semibold" style={{ backgroundImage:'var(--btn-grad)', color:'#fff' }}>{loading? 'Входим…' : 'Войти'}</button>
            </form>
          ...
          <>
            ...
            {step === 1 && (
              <form className="space-y-3" onSubmit={nextFromStep1}>
                <input required placeholder="Email" type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} className="w-full rounded-xl bg-white/5 border border-white/15 px-3 py-2 outline-none focus:border-white/30"/>
                <input required placeholder="Пароль" type="password" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} className="w-full rounded-xl bg-white/5 border border-white/15 px-3 py-2 outline-none focus:border-white/30"/>
                <button className="w-full rounded-xl px-4 py-2 font-semibold" style={{ backgroundImage:'var(--btn-grad)', color:'#fff' }}>Далее</button>
              </form>
            )}
            ...
            {step === 2 && (
              <form className="space-y-3" onSubmit={nextFromStep2}>
                ...
                <div className="flex gap-2">
                  ...
                  <button disabled={!form.agree} className="w-2/3 rounded-xl px-4 py-2 font-semibold" style={{ backgroundImage:'var(--btn-grad)', color:'#fff' }}>Далее</button>
                </div>
              </form>
            )}
            ...
            {step === 3 && (
              <form className="space-y-3" onSubmit={completeRegistration}>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="w-2/3 rounded-xl px-4 py-2 font-semibold" style={{ backgroundImage:'var(--btn-grad)', color:'#fff' }}>Завершить</button>
                </div>
              </form>
            )}
...
function ChangePlanModal({
  open, onClose, onPick, preset
}:{ open:boolean; onClose:()=>void; onPick?:(plan:string)=>void; preset?:string }){
  if(!open) return null;
  const plans = [
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md card rounded-2xl p-6 bg-[rgba(21,19,29,0.92)] border-white/20">
        ...
        <form className="space-y-3" onSubmit={(e)=>{ e.preventDefault(); onClose(); }}>
          ...
          <button className="w-full rounded-xl px-4 py-2 font-semibold" style={{ backgroundImage:'var(--btn-grad)', color:'#fff' }}>Оплатить и активировать</button>
        </form>
